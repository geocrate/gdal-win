...
use crate::gdal_sys::{CPLErr, GDALDriverH, GDALMajorObjectH};
...
impl Driver {
    ...
    pub fn short_name(&self) -> String {
        let rv = unsafe { crate::gdal_sys::GDALGetDriverShortName(self.c_driver) };
    ...
    pub fn long_name(&self) -> String {
        let rv = unsafe { crate::gdal_sys::GDALGetDriverLongName(self.c_driver) };
        _string(rv).unwrap_or_default()
    }
    ...
    fn _delete(&self, filename: &Path) -> Result<()> {
        ...
        let rv = unsafe { crate::gdal_sys::GDALDeleteDataset(self.c_driver, c_filename.as_ptr()) };
    ...
    fn _rename(&self, new_filename: &Path, old_filename: &Path) -> Result<()> {
        ...
        let rv = unsafe {
            crate::gdal_sys::GDALRenameDataset(
                self.c_driver,
...
impl DriverManager {
    ...
    pub fn count() -> usize {
        _register_drivers();
        let count = unsafe { crate::gdal_sys::GDALGetDriverCount() };
        count
            .try_into()
    ...
    pub fn get_driver(index: usize) -> Result<Driver> {
        _register_drivers();
        let c_driver = unsafe { crate::gdal_sys::GDALGetDriver(index.try_into().unwrap()) };
        if c_driver.is_null() {
    ...
    pub fn get_driver_by_name(name: &str) -> Result<Driver> {
        ...
        let c_driver = unsafe { crate::gdal_sys::GDALGetDriverByName(c_name.as_ptr()) };
        if c_driver.is_null() {
    ...
    pub fn register_driver(driver: &Driver) -> usize {
        let index = unsafe { crate::gdal_sys::GDALRegisterDriver(driver.c_driver) };
        index
            .try_into()
    ...
    pub fn deregister_driver(driver: &Driver) {
        unsafe {
            crate::gdal_sys::GDALDeregisterDriver(driver.c_driver);
        }
    }
    ...
    pub fn register_all() {
        unsafe {
            crate::gdal_sys::GDALAllRegister();
        }
    }
    ...
    pub fn destroy() {
        unsafe {
            crate::gdal_sys::GDALDestroyDriverManager();
        }
    }
