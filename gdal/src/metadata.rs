...
pub trait Metadata: MajorObject {
    ...
    fn description(&self) -> Result<String> {
        let c_res = unsafe { crate::gdal_sys::GDALGetDescription(self.gdal_object_ptr()) };
    ...
    fn metadata_domains(&self) -> Vec<String> {
        ...
        let c_res = unsafe { crate::gdal_sys::GDALGetMetadataDomainList(self.gdal_object_ptr()) };

        if !c_res.is_null() {
            domains.append(&mut _string_array(c_res));
        }
        unsafe { crate::gdal_sys::CSLDestroy(c_res) };
    ...
    fn metadata_domain(&self, domain: &str) -> Option<Vec<String>> {
        ...
        if let Ok(c_domain) = CString::new(domain.to_owned()) {
            let c_res =
                unsafe { crate::gdal_sys::GDALGetMetadata(self.gdal_object_ptr(), c_domain.as_ptr()) };
    ...
    fn metadata_item(&self, key: &str, domain: &str) -> Option<String> {
        if let Ok(c_key) = CString::new(key.to_owned()) {
            if let Ok(c_domain) = CString::new(domain.to_owned()) {
                let c_res = unsafe {
                    crate::gdal_sys::GDALGetMetadataItem(
    ...
    fn set_metadata_item(&mut self, key: &str, value: &str, domain: &str) -> Result<()> {
        ...
        let c_res = unsafe {
            crate::gdal_sys::GDALSetMetadataItem(
                self.gdal_object_ptr(),
                c_key.as_ptr(),
                c_value.as_ptr(),
                c_domain.as_ptr(),
    ...
    fn set_description(&mut self, description: &str) -> Result<()> {
        ...
        unsafe { crate::gdal_sys::GDALSetDescription(self.gdal_object_ptr(), c_description.as_ptr()) };
        Ok(())
    }
