...
impl Dataset {
    ...
    pub fn rasterband(&self, band_index: usize) -> Result<RasterBand> {
        ...
        unsafe {
            let c_band = crate::gdal_sys::GDALGetRasterBand(self.c_dataset(), band_index);
            if c_band.is_null() {
    ...
    pub fn build_overviews(
        ...
        let rv = unsafe {
            crate::gdal_sys::GDALBuildOverviews(
                self.c_dataset(),
                c_resampling.as_ptr(),
    ...
    pub fn raster_count(&self) -> usize {
        (unsafe { crate::gdal_sys::GDALGetRasterCount(self.c_dataset()) }) as usize
    }

    /// Returns the raster dimensions: (width, height).
    pub fn raster_size(&self) -> (usize, usize) {
        let size_x = unsafe { crate::gdal_sys::GDALGetRasterXSize(self.c_dataset()) } as usize;
        let size_y = unsafe { crate::gdal_sys::GDALGetRasterYSize(self.c_dataset()) } as usize;
        (size_x, size_y)
    }
}
...
///
/// For documentation, see `crate::gdal_sys::GDALRasterIOExtraArg`.
#[derive(Debug)]
#[allow(clippy::upper_case_acronyms)]
pub struct RasterIOExtraArg {
    pub n_version: usize,
    pub e_resample_alg: ResampleAlg,
    pub pfn_progress: crate::gdal_sys::GDALProgressFunc,
...
impl<'a> RasterBand<'a> {
    ...
    pub fn block_size(&self) -> (usize, usize) {
        ...
        unsafe { crate::gdal_sys::GDALGetBlockSize(self.c_rasterband, &mut size_x, &mut size_y) };
    ...
    pub fn x_size(&self) -> usize {
        let out;
        unsafe {
            out = crate::gdal_sys::GDALGetRasterBandXSize(self.c_rasterband);
        }
        out as usize
    }
    ...
    pub fn y_size(&self) -> usize {
        let out;
        unsafe { out = crate::gdal_sys::GDALGetRasterBandYSize(self.c_rasterband) }
        out as usize
    }
    ...
    pub fn band_type(&self) -> GdalDataType {
        let ordinal = unsafe { crate::gdal_sys::GDALGetRasterDataType(self.c_rasterband) };
    ...
    pub fn no_data_value(&self) -> Option<f64> {
        ...
        let no_data =
            unsafe { crate::gdal_sys::GDALGetRasterNoDataValue(self.c_rasterband, &mut pb_success) };
        if pb_success == 1 {
            return Some(no_data);
        }
        None
    ...
    pub fn set_no_data_value(&mut self, no_data: Option<f64>) -> Result<()> {
        let rv = if let Some(no_data) = no_data {
            unsafe { crate::gdal_sys::GDALSetRasterNoDataValue(self.c_rasterband, no_data) }
        } else {
            unsafe { crate::gdal_sys::GDALDeleteRasterNoDataValue(self.c_rasterband) }
        };
    ...
    pub fn no_data_value_u64(&self) -> Option<u64> {
        let mut pb_success = 1;
        let no_data = unsafe {
            crate::gdal_sys::GDALGetRasterNoDataValueAsUInt64(self.c_rasterband, &mut pb_success)
        };
        if pb_success == 1 {
            return Some(no_data);
        }
    ...
    pub fn set_no_data_value_u64(&mut self, no_data: Option<u64>) -> Result<()> {
        let rv = if let Some(no_data) = no_data {
            unsafe { crate::gdal_sys::GDALSetRasterNoDataValueAsUInt64(self.c_rasterband, no_data) }
        } else {
            unsafe { crate::gdal_sys::GDALDeleteRasterNoDataValue(self.c_rasterband) }
        };
    ...
    pub fn no_data_value_i64(&self) -> Option<i64> {
        let mut pb_success = 1;
        let no_data = unsafe {
            crate::gdal_sys::GDALGetRasterNoDataValueAsInt64(self.c_rasterband, &mut pb_success)
        };
        if pb_success == 1 {
            return Some(no_data);
        }
    ...
    pub fn set_no_data_value_i64(&mut self, no_data: Option<i64>) -> Result<()> {
        let rv = if let Some(no_data) = no_data {
            unsafe { crate::gdal_sys::GDALSetRasterNoDataValueAsInt64(self.c_rasterband, no_data) }
        } else {
            unsafe { crate::gdal_sys::GDALDeleteRasterNoDataValue(self.c_rasterband) }
        };
    ...
    pub fn color_interpretation(&self) -> ColorInterpretation {
        let interp_index = unsafe { crate::gdal_sys::GDALGetRasterColorInterpretation(self.c_rasterband) };
    ...
    pub fn set_color_interpretation(&mut self, interp: ColorInterpretation) -> Result<()> {
        ...
        let rv =
            unsafe { crate::gdal_sys::GDALSetRasterColorInterpretation(self.c_rasterband, interp_index) };
        if rv != CPLErr::CE_None {
            return Err(_last_cpl_err(rv));
        }
        Ok(())
    ...
    pub fn color_table(&self) -> Option<ColorTable> {
        let c_color_table = unsafe { crate::gdal_sys::GDALGetRasterColorTable(self.c_rasterband) };
    ...
    pub fn scale(&self) -> Option<f64> {
        let mut pb_success = 1;
        let scale = unsafe { crate::gdal_sys::GDALGetRasterScale(self.c_rasterband, &mut pb_success) };
        if pb_success == 1 {
            return Some(scale);
        }
        None
    ...
    pub fn set_scale(&mut self, scale: f64) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::GDALSetRasterScale(self.c_rasterband, scale) };
        if rv != CPLErr::CE_None {
            return Err(_last_cpl_err(rv));
        }
        Ok(())
    ...
    pub fn offset(&self) -> Option<f64> {
        let mut pb_success = 1;
        let offset = unsafe { crate::gdal_sys::GDALGetRasterOffset(self.c_rasterband, &mut pb_success) };
        if pb_success == 1 {
            return Some(offset);
        }
        None
    ...
    pub fn set_offset(&mut self, offset: f64) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::GDALSetRasterOffset(self.c_rasterband, offset) };
        if rv != CPLErr::CE_None {
            return Err(_last_cpl_err(rv));
        }
        Ok(())
    ...
    pub fn actual_block_size(&self, x: usize, y: usize) -> Result<(usize, usize)> {
        ...
        let rv = unsafe {
            crate::gdal_sys::GDALGetActualBlockSize(
                self.c_rasterband,
                offset_x,
                offset_y,
                &mut block_size_x,
    ...
    pub fn overview_count(&self) -> Result<i32> {
        unsafe { Ok(crate::gdal_sys::GDALGetOverviewCount(self.c_rasterband)) }
    }
    ...
    pub fn overview(&self, overview_index: usize) -> Result<RasterBand<'a>> {
        ...
        unsafe {
            ...
            let overview = crate::gdal_sys::GDALGetOverview(c_band, overview_index);
            if overview.is_null() {
    ...
    pub fn unit(&self) -> String {
        let c_ptr = unsafe { crate::gdal_sys::GDALGetRasterUnitType(self.c_rasterband) };
        _string(c_ptr).unwrap_or_default()
    }

    /// Read the band mask flags for a GDAL `RasterBand`.
    pub fn mask_flags(&self) -> Result<GdalMaskFlags> {
        let band_mask_flags = unsafe { crate::gdal_sys::GDALGetMaskFlags(self.c_rasterband) };
    ...
    pub fn create_mask_band(&mut self, shared_between_all_bands: bool) -> Result<()> {
        ...
        let rv = unsafe { crate::gdal_sys::GDALCreateMaskBand(self.c_rasterband, flags) };
        if rv != 0 {
            return Err(_last_cpl_err(rv));
        };
        Ok(())
    ...
    pub fn open_mask_band(&self) -> Result<RasterBand> {
        unsafe {
            let mask_band_ptr = crate::gdal_sys::GDALGetMaskBand(self.c_rasterband);
...
///   stored in `HistogramCounts::GdalAllocated`, to be deallocated with [`VSIFree`][crate::gdal_sys::VSIFree].
enum HistogramCounts {
    /// Pointer to GDAL allocated array and length of histogram counts.
    ///
    /// Requires freeing with [`VSIFree`][crate::gdal_sys::VSIFree].
    ...
    fn drop(&mut self) {
        match self {
            HistogramCounts::GdalAllocated(p, _) => unsafe {
                crate::gdal_sys::VSIFree(*p as *mut c_void);
            },
...
impl ColorInterpretation {
    ...
    pub fn from_name(name: &str) -> Result<Self> {
        ...
        let interp_index =
            unsafe { crate::gdal_sys::GDALGetColorInterpretationByName(c_str_interp_name.as_ptr()) };
        Ok(Self::from_c_int(interp_index).unwrap())
    }

    /// Returns the name of this color interpretation.
    pub fn name(&self) -> String {
        let c_ptr = unsafe { crate::gdal_sys::GDALGetColorInterpretationName(self.c_int()) };
        _string(c_ptr).unwrap_or_default()
    }
}
...
impl<'a> ColorTable<'a> {
    ...
    pub fn entry_count(&self) -> usize {
        unsafe { crate::gdal_sys::GDALGetColorEntryCount(self.c_color_table) as usize }
    }

    /// Get a color entry.
    pub fn entry(&self, index: usize) -> Option<ColorEntry> {
        let color_entry = unsafe {
            let c_color_entry = crate::gdal_sys::GDALGetColorEntry(self.c_color_table, index as i32);
    ...
    pub fn entry_as_rgb(&self, index: usize) -> Option<RgbaEntry> {
        ...
        if unsafe {
            crate::gdal_sys::GDALGetColorEntryAsRGB(self.c_color_table, index as i32, &mut color_entry)
        } == 0
        {
            return None;
        }
