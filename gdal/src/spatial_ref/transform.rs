...
use crate::gdal_sys::{CPLErr, OGRCoordinateTransformationH};
...
impl Drop for CoordTransform {
    fn drop(&mut self) {
        unsafe { crate::gdal_sys::OCTDestroyCoordinateTransformation(self.inner) };
    }
}
...
impl CoordTransform {
    ...
    pub fn new(source: &SpatialRef, target: &SpatialRef) -> errors::Result<CoordTransform> {
        let c_obj = unsafe {
            crate::gdal_sys::OCTNewCoordinateTransformation(source.to_c_hsrs(), target.to_c_hsrs())
        };
        if c_obj.is_null() {
    ...
    pub fn new_with_options(
        ...
        let c_obj = unsafe {
            crate::gdal_sys::OCTNewCoordinateTransformationEx(
                source.to_c_hsrs(),
                target.to_c_hsrs(),
                options.c_options(),
            )
    ...
    pub fn transform_bounds(
        ...
        let ret_val = unsafe {
            crate::gdal_sys::OCTTransformBounds(
                self.inner,
                bounds[0],
                bounds[1],
                bounds[2],
    ...
    /// Returns a C pointer to the allocated [`crate::gdal_sys::OGRCoordinateTransformationH`] memory.
    ///
    /// # Safety
