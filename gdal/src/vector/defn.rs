...
impl Defn {
    ...
    pub fn fields(&self) -> FieldIterator {
        let total = unsafe { crate::gdal_sys::OGR_FD_GetFieldCount(self.c_defn) } as isize;
        FieldIterator {
            defn: self,
    ...
    pub fn geom_fields(&self) -> GeomFieldIterator {
        let total = unsafe { crate::gdal_sys::OGR_FD_GetGeomFieldCount(self.c_defn) } as isize;
        GeomFieldIterator {
            defn: self,
            c_feature_defn: self.c_defn,
            next_id: 0,
    ...
    pub fn from_layer<L: LayerAccess>(lyr: &L) -> Defn {
        let c_defn = unsafe { crate::gdal_sys::OGR_L_GetLayerDefn(lyr.c_layer()) };
        Defn { c_defn }
    }

    /// Get the geometry type of the first geometry field
    pub fn geometry_type(&self) -> OGRwkbGeometryType::Type {
        unsafe { crate::gdal_sys::OGR_FD_GetGeomType(self.c_defn) }
    }
    ...
    fn _field_index(&self, field_name: &str) -> Result<usize> {
        ...
        let field_idx =
            unsafe { crate::gdal_sys::OGR_FD_GetFieldIndex(self.c_defn(), c_str_field_name.as_ptr()) };
        if field_idx == -1 {
    ...
    fn _geometry_field_index(&self, field_name: &str) -> Result<usize> {
        let c_str_field_name = CString::new(field_name)?;
        let field_idx =
            unsafe { crate::gdal_sys::OGR_FD_GetGeomFieldIndex(self.c_defn(), c_str_field_name.as_ptr()) };
        if field_idx == -1 {
            return Err(GdalError::InvalidFieldName {
                field_name: field_name.to_string(),
...
impl<'a> Iterator for FieldIterator<'a> {
    ...
    fn next(&mut self) -> Option<Field<'a>> {
        ...
        let field = Field {
            _defn: self.defn,
            c_field_defn: unsafe {
                crate::gdal_sys::OGR_FD_GetFieldDefn(self.c_feature_defn, self.next_id as c_int)
            },
        };
        self.next_id += 1;
        Some(field)
...
impl<'a> Field<'a> {
    ...
    pub fn name(&'a self) -> String {
        let rv = unsafe { crate::gdal_sys::OGR_Fld_GetNameRef(self.c_field_defn) };
        _string(rv).unwrap_or_default()
    }

    /// Get the alternative name (alias) of this field.
    pub fn alternative_name(&'a self) -> String {
        let rv = unsafe { crate::gdal_sys::OGR_Fld_GetAlternativeNameRef(self.c_field_defn) };
        _string(rv).unwrap_or_default()
    }

    /// Get the data type of this field.
    pub fn field_type(&'a self) -> OGRFieldType::Type {
        unsafe { crate::gdal_sys::OGR_Fld_GetType(self.c_field_defn) }
    }
    ...
    pub fn width(&'a self) -> i32 {
        unsafe { crate::gdal_sys::OGR_Fld_GetWidth(self.c_field_defn) }
    }
    ...
    pub fn precision(&'a self) -> i32 {
        unsafe { crate::gdal_sys::OGR_Fld_GetPrecision(self.c_field_defn) }
    }

    /// Return whether this field can receive null values.
    pub fn is_nullable(&'a self) -> bool {
        unsafe { crate::gdal_sys::OGR_Fld_IsNullable(self.c_field_defn) != 0 }
    }

    /// Return whether this field has a unique constraint.
    pub fn is_unique(&'a self) -> bool {
        unsafe { crate::gdal_sys::OGR_Fld_IsUnique(self.c_field_defn) != 0 }
    }

    /// Get default field value.
    pub fn default_value(&'a self) -> Option<String> {
        let c_ptr = unsafe { crate::gdal_sys::OGR_Fld_GetDefault(self.c_field_defn) };
        _string(c_ptr)
    }
}
...
impl<'a> Iterator for GeomFieldIterator<'a> {
    ...
    fn next(&mut self) -> Option<GeomField<'a>> {
        ...
        let field = GeomField {
            _defn: self.defn,
            c_field_defn: unsafe {
                crate::gdal_sys::OGR_FD_GetGeomFieldDefn(self.c_feature_defn, self.next_id as c_int)
            },
        };
        self.next_id += 1;
        Some(field)
...
impl<'a> GeomField<'a> {
    /// Get the name of this field.
    pub fn name(&'a self) -> String {
        let rv = unsafe { crate::gdal_sys::OGR_GFld_GetNameRef(self.c_field_defn) };
        _string(rv).unwrap_or_default()
    }

    pub fn field_type(&'a self) -> OGRwkbGeometryType::Type {
        unsafe { crate::gdal_sys::OGR_GFld_GetType(self.c_field_defn) }
    }

    pub fn spatial_ref(&'a self) -> Result<SpatialRef> {
        let c_obj = unsafe { crate::gdal_sys::OGR_GFld_GetSpatialRef(self.c_field_defn) };
        if c_obj.is_null() {
