...
use crate::gdal_sys::{OGRErr, OGRFeatureH, OGRFieldType, OGRLayerH};
...
impl<'a> Feature<'a> {
    pub fn new(defn: &'a Defn) -> Result<Feature<'a>> {
        let c_feature = unsafe { crate::gdal_sys::OGR_F_Create(defn.c_defn()) };
    ...
    pub fn _lazy_feature_geometries(defn: &'a Defn) -> Vec<Geometry> {
        let geom_field_count =
            unsafe { crate::gdal_sys::OGR_FD_GetGeomFieldCount(defn.c_defn()) } as isize;
        (0..geom_field_count)
    ...
    pub fn fid(&self) -> Option<u64> {
        let fid = unsafe { crate::gdal_sys::OGR_F_GetFID(self.c_feature) };
        if fid < 0 {
            None
        } else {
            Some(fid as u64)
    ...
    pub fn field(&self, field_idx: usize) -> Result<Option<FieldValue>> {
        ...
        if unsafe { crate::gdal_sys::OGR_F_IsFieldNull(self.c_feature, field_idx) } != 0 {
            return Ok(None);
        }

        let field_defn = unsafe { crate::gdal_sys::OGR_F_GetFieldDefnRef(self.c_feature, field_idx) };
        let field_type = unsafe { crate::gdal_sys::OGR_Fld_GetType(field_defn) };
        match field_type {
            OGRFieldType::OFTString => {
                let rv = unsafe { crate::gdal_sys::OGR_F_GetFieldAsString(self.c_feature, field_idx) };
                Ok(_string(rv).map(FieldValue::StringValue))
            }
            OGRFieldType::OFTStringList => {
                let rv = unsafe {
                    let ptr = crate::gdal_sys::OGR_F_GetFieldAsStringList(self.c_feature, field_idx);
                    _string_array(ptr)
                };
                Ok(Some(FieldValue::StringListValue(rv)))
            }
            OGRFieldType::OFTReal => {
                let rv = unsafe { crate::gdal_sys::OGR_F_GetFieldAsDouble(self.c_feature, field_idx) };
            ...
            OGRFieldType::OFTRealList => {
                let rv = unsafe {
                    let mut len: i32 = 0;
                    let ptr =
                        crate::gdal_sys::OGR_F_GetFieldAsDoubleList(self.c_feature, field_idx, &mut len);
                    slice::from_raw_parts(ptr, len as usize).to_vec()
                };
                Ok(Some(FieldValue::RealListValue(rv)))
            }
            OGRFieldType::OFTInteger => {
                let rv = unsafe { crate::gdal_sys::OGR_F_GetFieldAsInteger(self.c_feature, field_idx) };
            ...
            OGRFieldType::OFTIntegerList => {
                let rv = unsafe {
                    let mut len: i32 = 0;
                    let ptr =
                        crate::gdal_sys::OGR_F_GetFieldAsIntegerList(self.c_feature, field_idx, &mut len);
                    slice::from_raw_parts(ptr, len as usize).to_vec()
                };
                Ok(Some(FieldValue::IntegerListValue(rv)))
            }
            OGRFieldType::OFTInteger64 => {
                let rv = unsafe { crate::gdal_sys::OGR_F_GetFieldAsInteger64(self.c_feature, field_idx) };
                Ok(Some(FieldValue::Integer64Value(rv)))
            }
            OGRFieldType::OFTInteger64List => {
                let rv = unsafe {
                    let mut len: i32 = 0;
                    let ptr = crate::gdal_sys::OGR_F_GetFieldAsInteger64List(
                        self.c_feature,
                        field_idx,
                        &mut len,
                    );
    ...
    fn _field_index(&self, field_name: &str) -> Result<usize> {
        ...
        let field_idx =
            unsafe { crate::gdal_sys::OGR_F_GetFieldIndex(self.c_feature(), c_str_field_name.as_ptr()) };
        if field_idx == -1 {
    ...
    fn _geometry_field_index(&self, field_name: &str) -> Result<usize> {
        ...
        let field_idx = unsafe {
            crate::gdal_sys::OGR_F_GetGeomFieldIndex(self.c_feature(), c_str_field_name.as_ptr())
        };
        if field_idx == -1 {
            return Err(GdalError::InvalidFieldName {
                field_name: field_name.to_string(),
    ...
    pub fn field_as_integer(&self, field_idx: usize) -> Result<Option<i32>> {
        ...
        if unsafe { crate::gdal_sys::OGR_F_IsFieldNull(self.c_feature, idx) } != 0 {
            return Ok(None);
        }

        let value = unsafe { crate::gdal_sys::OGR_F_GetFieldAsInteger(self.c_feature, idx) };
    ...
    pub fn field_as_integer64(&self, field_idx: usize) -> Result<Option<i64>> {
        ...
        let value = unsafe { crate::gdal_sys::OGR_F_GetFieldAsInteger64(self.c_feature, idx) };
    ...
    pub fn field_as_double(&self, field_idx: usize) -> Result<Option<f64>> {
        ...
        let value = unsafe { crate::gdal_sys::OGR_F_GetFieldAsDouble(self.c_feature, idx) };
    ...
    pub fn field_as_string(&self, field_idx: usize) -> Result<Option<String>> {
        ...
        let value = _string(unsafe { crate::gdal_sys::OGR_F_GetFieldAsString(self.c_feature, idx) });
        Ok(value)
    }
    ...
    fn _field_as_datetime(&self, field_id: c_int) -> Result<DateTime<FixedOffset>> {
        ...
        let success = unsafe {
            crate::gdal_sys::OGR_F_GetFieldAsDateTime(
                self.c_feature,
                field_id,
                &mut year,
                &mut month,
        ...
        match self.geometry.first() {
            Some(geom) => {
                if !geom.has_gdal_ptr() {
                    let c_geom = unsafe { crate::gdal_sys::OGR_F_GetGeometryRef(self.c_feature) };
    ...
    pub fn geometry_by_index(&self, idx: usize) -> Result<&Geometry> {
        ...
        if !self.geometry[idx].has_gdal_ptr() {
            let c_geom = unsafe { crate::gdal_sys::OGR_F_GetGeomFieldRef(self.c_feature, idx as i32) };
            if c_geom.is_null() {
    ...
    pub fn create<L: LayerAccess>(&self, lyr: &L) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::OGR_L_CreateFeature(lyr.c_layer(), self.c_feature) };
    ...
    pub fn set_field_string(&mut self, field_idx: usize, value: &str) -> Result<()> {
        ...
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_SetFieldString(self.c_feature, idx, c_str_value.as_ptr()) };
        Ok(())
    }
    ...
    pub fn set_field_string_list(&mut self, field_idx: usize, value: &[&str]) -> Result<()> {
        ...
        unsafe { crate::gdal_sys::OGR_F_SetFieldStringList(self.c_feature, idx, c_value) };
        Ok(())
    }

    pub fn set_field_double(&mut self, field_idx: usize, value: f64) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_SetFieldDouble(self.c_feature, idx, value as c_double) };
        Ok(())
    }
    ...
    pub fn set_field_double_list(&mut self, field_idx: usize, value: &[f64]) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe {
            crate::gdal_sys::OGR_F_SetFieldDoubleList(
                self.c_feature,
                idx,
                value.len() as c_int,
                value.as_ptr(),
    ...
    pub fn set_field_integer(&mut self, field_idx: usize, value: i32) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_SetFieldInteger(self.c_feature, idx, value as c_int) };
        Ok(())
    }
    ...
    pub fn set_field_integer_list(&mut self, field_idx: usize, value: &[i32]) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe {
            crate::gdal_sys::OGR_F_SetFieldIntegerList(
                self.c_feature,
                idx,
                value.len() as c_int,
                value.as_ptr(),
    ...
    pub fn set_field_integer64(&mut self, field_idx: usize, value: i64) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_SetFieldInteger64(self.c_feature, idx, value as c_longlong) };
        Ok(())
    }
    ...
    pub fn set_field_integer64_list(&mut self, field_idx: usize, value: &[i64]) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe {
            crate::gdal_sys::OGR_F_SetFieldInteger64List(
                self.c_feature,
                idx,
                value.len() as c_int,
                value.as_ptr(),
    ...
    pub fn set_field_datetime(
        ...
        unsafe {
            crate::gdal_sys::OGR_F_SetFieldDateTime(
                self.c_feature,
                idx,
                year,
                month,
    ...
    pub fn set_field_null(&mut self, field_idx: usize) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_SetFieldNull(self.c_feature(), idx) };
        Ok(())
    }
    ...
    pub fn unset_field(&mut self, field_idx: usize) -> Result<()> {
        let idx = field_idx.try_into()?;
        unsafe { crate::gdal_sys::OGR_F_UnsetField(self.c_feature(), idx) };
        Ok(())
    }

    pub fn set_geometry(&mut self, geom: Geometry) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::OGR_F_SetGeometry(self.c_feature, geom.c_geometry()) };
        if rv != OGRErr::OGRERR_NONE {
            return Err(GdalError::OgrError {
                err: rv,
    ...
    pub fn field_count(&self) -> usize {
        let count = unsafe { crate::gdal_sys::OGR_F_GetFieldCount(self.c_feature) };
        count as usize
    }
...
impl Iterator for FieldValueIterator<'_> {
    ...
    fn next(&mut self) -> Option<(String, Option<FieldValue>)> {
        ...
        if idx < self.count {
            self.idx += 1;
            let field_defn =
                unsafe { crate::gdal_sys::OGR_F_GetFieldDefnRef(self.feature.c_feature, idx as c_int) };
            let field_name = unsafe { crate::gdal_sys::OGR_Fld_GetNameRef(field_defn) };
...
impl Drop for Feature<'_> {
    fn drop(&mut self) {
        unsafe {
            crate::gdal_sys::OGR_F_Destroy(self.c_feature);
        }
    }
}
...
impl<'a> Iterator for FeatureIterator<'a> {
    ...
    fn next(&mut self) -> Option<Feature<'a>> {
        let c_feature = unsafe { crate::gdal_sys::OGR_L_GetNextFeature(self.c_layer) };
        if c_feature.is_null() {
            None
        } else {
    ...
    Self: 'a,
{
    ...
    #[inline]
    fn next(&mut self) -> Option<Feature<'a>> {
        let c_feature = unsafe { crate::gdal_sys::OGR_L_GetNextFeature(self.layer.c_layer()) };
...
pub fn field_type_to_name(ty: OGRFieldType::Type) -> String {
    let rv = unsafe { crate::gdal_sys::OGR_GetFieldTypeName(ty) };
