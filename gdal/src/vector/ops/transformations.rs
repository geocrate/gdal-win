...
impl Geometry {
    ...
    pub fn transform_inplace(&mut self, htransform: &CoordTransform) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::OGR_G_Transform(self.c_geometry(), htransform.to_c_hct()) };
    ...
    pub fn transform(&self, htransform: &CoordTransform) -> Result<Geometry> {
        let new_c_geom = unsafe { crate::gdal_sys::OGR_G_Clone(self.c_geometry()) };
        let rv = unsafe { crate::gdal_sys::OGR_G_Transform(new_c_geom, htransform.to_c_hct()) };
        if rv != OGRErr::OGRERR_NONE {
            return Err(GdalError::OgrError {
                err: rv,
                method_name: "OGR_G_Transform",
    ...
    pub fn transform_to_inplace(&mut self, spatial_ref: &SpatialRef) -> Result<()> {
        let rv = unsafe { crate::gdal_sys::OGR_G_TransformTo(self.c_geometry(), spatial_ref.to_c_hsrs()) };
        if rv != OGRErr::OGRERR_NONE {
            return Err(GdalError::OgrError {
                err: rv,
    ...
    pub fn transform_to(&self, spatial_ref: &SpatialRef) -> Result<Geometry> {
        let new_c_geom = unsafe { crate::gdal_sys::OGR_G_Clone(self.c_geometry()) };
        let rv = unsafe { crate::gdal_sys::OGR_G_TransformTo(new_c_geom, spatial_ref.to_c_hsrs()) };
        if rv != OGRErr::OGRERR_NONE {
            return Err(GdalError::OgrError {
                err: rv,
                method_name: "OGR_G_TransformTo",
    ...
    pub fn convex_hull(&self) -> Result<Geometry> {
        let c_geom = unsafe { crate::gdal_sys::OGR_G_ConvexHull(self.c_geometry()) };
        if c_geom.is_null() {
    ...
    pub fn delaunay_triangulation(&self, tolerance: Option<f64>) -> Result<Self> {
        let c_geom = unsafe {
            crate::gdal_sys::OGR_G_DelaunayTriangulation(self.c_geometry(), tolerance.unwrap_or(0.0), 0)
        };
        if c_geom.is_null() {
    ...
    pub fn simplify(&self, tolerance: f64) -> Result<Self> {
        let c_geom = unsafe { crate::gdal_sys::OGR_G_Simplify(self.c_geometry(), tolerance) };
        if c_geom.is_null() {
    ...
    pub fn simplify_preserve_topology(&self, tolerance: f64) -> Result<Self> {
        let c_geom =
            unsafe { crate::gdal_sys::OGR_G_SimplifyPreserveTopology(self.c_geometry(), tolerance) };
        if c_geom.is_null() {
    ...
    pub fn buffer(&self, distance: f64, n_quad_segs: u32) -> Result<Self> {
        let c_geom =
            unsafe { crate::gdal_sys::OGR_G_Buffer(self.c_geometry(), distance, n_quad_segs as i32) };
        if c_geom.is_null() {
    ...
    pub fn make_valid(&self, opts: &CslStringList) -> Result<Geometry> {
        let c_geom = unsafe { crate::gdal_sys::OGR_G_MakeValidEx(self.c_geometry(), opts.as_ptr()) };
...
mod tests {
    ...
    pub fn test_buffer() {
        ...
        assert_eq!(
            ...
            crate::gdal_sys::OGRwkbGeometryType::wkbPolygon
        );
