...
use crate::gdal_sys::{VSIFCloseL, VSIFileFromMemBuffer, VSIFree, VSIGetMemFileBuffer, VSIUnlink};
...
pub fn read_dir<P: AsRef<Path>>(path: P, recursive: bool) -> Result<Vec<PathBuf>> {
    fn _read_dir(path: &Path, recursive: bool) -> Result<Vec<PathBuf>> {
        ...
        let data = if recursive {
            let data = unsafe { crate::gdal_sys::VSIReadDirRecursive(path.as_ptr()) };
            if data.is_null() {
                return Err(_last_null_pointer_err("VSIReadDirRecursive"));
            }
            data
        } else {
            let data = unsafe { crate::gdal_sys::VSIReadDir(path.as_ptr()) };
            if data.is_null() {
        ...
        unsafe { crate::gdal_sys::CSLDestroy(data) };
        ret
    }
